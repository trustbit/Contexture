  ...
  <div class="flex flex-col gap-3">
    <div class="rounded bg-white p-4">
      ...
      <ul class="mt-2 space-y-2">
        <ContextureListItem v-for="message in messages?.commandsHandled" :key="message">
          <template #title>
            <span class="text-sm font-bold text-gray-900">{{ message }}</span>
            <button @click="() => onDeleteMessage('commandsHandled', message)">
      ...
      <ContextureCollapsable
        ...
        :collapsed="collapsedState.commandsHandled"
        data-testId="addCommandHandled"
    ...
    <div class="rounded bg-white p-4">
      ...
      <ul class="mt-2 space-y-2">
        <ContextureListItem v-for="message in messages?.eventsHandled" :key="message">
          <template #title>
            <span class="text-sm font-bold text-gray-900">{{ message }}</span>
            <button @click="() => onDeleteMessage('eventsHandled', message)">
      ...
      <ContextureCollapsable
        ...
        class="mt-3"
        :cancel-text="t('common.cancel')"
        :collapsed="collapsedState.eventsHandled"
        data-testId="addEventHandled"
    ...
    <div class="rounded bg-white p-4">
      ...
      <ul class="mt-2 space-y-2">
        <ContextureListItem v-for="message in messages?.queriesHandled" :key="message">
          <template #title>
            <span class="text-sm font-bold text-gray-900">{{ message }}</span>
            <button @click="() => onDeleteMessage('queriesHandled', message)">
      ...
      <ContextureCollapsable
        ...
        class="mt-3"
        :cancel-text="t('common.cancel')"
        :collapsed="collapsedState.queriesHandled"
        data-testId="addQueryHandled"
...
const collapsedState = ref<{ [key in MessagesConsumedKeys]: boolean }>({
...
async function onAddMessage(key: MessagesConsumedKeys, createMessage: CreateMessage) {
  const res = await store.addMessage(activeBoundedContext.value.id, key, createMessage.name);
...
function updateCollapsed(key: MessagesConsumedKeys, collapsed: boolean) {
