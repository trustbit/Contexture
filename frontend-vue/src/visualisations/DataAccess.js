function selectAllParentDomains(domains, relevantDomainsIds) {
  ...
  do {
    newDomainIds = Array.from(relevantDomainsIds.values())
      .map((domainId) => {
        ...
        if (domain && domain.parentDomainId && !relevantDomainsIds.has(domain.parentDomainId)) {
...
async function fetchDomains(baseApi) {
  const response = await fetch(`${baseApi}/api/domains`);

  return await response.json();
}

async function fetchBoundedContexts(baseApi, query) {
  const response = await fetch(`${baseApi}/api/boundedContexts${query || ""}`);
...
export async function fetchData(baseApi, query, highlightMode) {
  ...
  const displayContexts = highlightMode ? await fetchBoundedContexts(baseApi) : filteredContexts;
  const domains = await fetchDomains(baseApi);

  const boundedContextsToDisplay = Array.from(displayContexts).reduce((dict, bc) => {
    if (!dict[bc.parentDomainId]) dict[bc.parentDomainId] = [];
    dict[bc.parentDomainId].push(bc);
    return dict;
  }, {});

  const foundBoundedContextIds = new Set(Array.from(filteredContexts).map((context) => context.id));

  const foundDomainIds = new Set(Array.from(filteredContexts).map((context) => context.parentDomainId));

  // starting from the domains of the matched bounded contexts
  const relevantDomainsIds = selectAllParentDomains(domains, foundDomainIds);
