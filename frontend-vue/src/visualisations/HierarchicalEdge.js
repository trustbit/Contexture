...
function initializeElements(element, { diameter }) {
  ...
  const group = svg.append("g").attr("font-size", 10).attr("font-family", "sans-serif").selectAll("g");

  const lines = svg.append("g").attr("fill-opacity", 0.75).selectAll("g");
...
function guessWidthAndHeightFromElement(element) {
  ...
  const maxHeight = Math.max(
    400,
    element.clientHeight - parseFloat(parentStyle.paddingTop) - parseFloat(parentStyle.paddingBottom)
  );
  return {
    width: width,
    maxHeight: maxHeight,
...
function packageMatrix(collaborations, domains, existIds) {
  ...
  function resolveCollaboratorName(collaborator) {
    const boundedContexts = domains.map((domain) => domain.boundedContexts).flat();
    ...
    if (collaborator.boundedContext) {
      ...
      const domainNames = resolveDomainNames(boundedContext.parentDomainId).join(".");

      return `${domainNames}.${boundedContext.shortName || boundedContext.name}`;
    }

    if (collaborator.domain) {
      const domain = domains.find((domain) => domain.id == collaborator.domain);
    ...
    throw new Error(`Could not resolve a name for collaborator ${JSON.stringify(collaborator)}`);
  }
  ...
  const collaborationNames = unique(
    collaborations
      .map((collaboration) => [collaboration.initiator, collaboration.recipient])
      .flat()
  ...
  function buildMatrix(data, names) {
    ...
    for (const { source, target, value } of data) {
      const matrixLocal = matrix[index.get(source)];
      if (matrixLocal) {
        matrixLocal[index.get(target)] += value;
      }
    }
...
export class HierarchicalEdge extends HTMLElement {
  ...
  async buildHierarchicalEdge() {
    ...
    const baseApi = import.meta.env.VITE_CONTEXTURE_API_BASE_URL;
    ...
    const collaborationResponse = await fetch(`/api/collaborations`);
  ...
  renderCollaborations() {
    ...
    const group = this.matrixEdge.group.data(chords.groups).join("g");
  ...
  mouseovered(rootNode, d) {
    ...
    rootNode.selectAll(".chord").style("opacity", 0.1);
    rootNode.selectAll(`.chord[sourceIndex="${index}"]`).style("opacity", 1);

    rootNode.selectAll("text.group").style("opacity", 0.1);
