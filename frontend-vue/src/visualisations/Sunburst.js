...
function guessWidthAndHeightFromElement(element) {
  ...
  const maxHeight = element.clientHeight - parseFloat(parentStyle.paddingTop) - parseFloat(parentStyle.paddingBottom);
...
export class Sunburst extends HTMLElement {
  ...
  async buildSunburst() {
    ...
    this.data = await fetchData(baseUrl, query, this.inHighlightMode());
  ...
  renderSunburst() {
    ...
    const color = d3.scaleOrdinal(d3.quantize(d3.interpolateRainbow, root.children ? root.children.length + 1 : 0));
    ...
    function highlightOpacity(d) {
      if (inHighlightMode) {
        if (d.data.wasFound) {
          return d.data.isBoundedContext ? 1.0 : 0.7;
        } else {
          return 0.3;
        }
      } else {
    ...
    const elements = this.sunburst.elements
      ...
      .join((enter) => {
        ...
        a.attr("target", "_blank").append("path").append("title");

        return a;
      });

    elements.attr("xlink:href", (d) =>
      d.data.isBoundedContext ? `/boundedContext/${d.data.id}/canvas` : `/domain/${d.data.id}`
    );
    elements
      .select("path")
      .attr("fill", (d) => {
